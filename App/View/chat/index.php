...
<body class="chat-body">
    ...
    function renderMessage(m) {
        ...
        if (m.type === 'voice') {
            div.innerHTML =
                `<div class="message-content voice"><audio controls style="max-width:200px"><source src="${m.message}"></audio><span class="message-time">${time}</span></div>`;
        } else {
            div.innerHTML =
                `<div class="message-content"><p>${m.message}</p><span class="message-time">${time}</span></div>`;
        }
    ...
    voiceBtn.addEventListener('click', async function() {
        ...
        try {
            ...
            const types = ['audio/mp4;codecs=mp4a.40.2', 'audio/mp4', 'audio/aac', 'audio/webm;codecs=opus',
                'audio/webm', 'audio/ogg;codecs=opus'
            ];
            const preferred = types.find(t => MediaRecorder.isTypeSupported(t)) || '';
            mediaRecorder = new MediaRecorder(stream, preferred ? {
                mimeType: preferred
            } : {});
            audioChunks = [];
            ...
            mediaRecorder.onstop = () => {
                ...
                const ext = actual.includes('mp4') || actual.includes('aac') ? 'm4a' : actual.includes(
                    'ogg') ? 'ogg' : 'webm';
